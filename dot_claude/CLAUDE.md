...
- On Tailscale as `franks-mac-studio.tail3f1556.ts.net` (tailnet-only HTTPS). Serve allows ports 443, 8443, 10000 only: **443 = the repo being live-edited, 8443 + 10000 = card previews.** Each app gets its own port; never serve an SPA from a sub-path.
...
- Rails dev apps need the tailnet host allowed: `config.hosts << /\A[a-z0-9-]+\.[a-z0-9-]+\.ts\.net\z/` (the `".ts.net"` shorthand only admits one subdomain level). Google sign-in needs each `https://…ts.net[:port]` registered as an OAuth JavaScript origin.
